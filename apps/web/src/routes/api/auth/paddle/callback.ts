...
export async function GET(request: Request) {
  ...
  // In production: use Secure flag and proper Max-Age
  ...
  // 1. Using proper session tokens (not JSON in cookie)
  // 2. Storing session data server-side
  // 3. Using JWT with proper signing
  const response = new Response(null, {
    status: 302,
    headers: {
      Location: '/',
      'Set-Cookie': `session=${encodeURIComponent(sessionToken)}; Path=/; HttpOnly; SameSite=Lax${secureFlag}; Max-Age=${maxAge}`,
    },
  })
