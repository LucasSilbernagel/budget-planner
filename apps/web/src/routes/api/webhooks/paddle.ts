...
import { db, currencyEnum } from '@budget-planner/db'
import { users, type Currency, type SubscriptionStatus } from '@budget-planner/db/src/schema'
...
async function handleSubscriptionStatusUpdate(
  ...
  const mappedCurrency: Currency = currency && currencyValues.includes(currency.toUpperCase())
