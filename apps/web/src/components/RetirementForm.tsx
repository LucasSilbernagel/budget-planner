...
function RetirementFormInner({ onCalculate, preFillFromExistingData = true, savingsRate = 0.5 }: RetirementFormProps) {
  ...
  const retirementInsights = useMemo(() => {
    ...
    const gapPercentage = (requiredAssets > 0 && totalInvestmentAssets > 0)
    ...
    const yearsToGoal = (monthlyIncomeCents > 0 && parsedReturnRate > 0)
  ...
  }, [requiredAssets, totalInvestmentAssets, monthlyIncomeInput, annualReturnRateInput, calculateYearsToGoal])
  ...
  const sanitizeDisplayError = (error: unknown): string => {
    ...
    const userFriendlyMessages: Record<string, string> = {
      'Annual return rate must be positive (greater than 0)': 'Please enter a valid return rate (must be greater than 0%)',
      'Annual return rate must be positive (greater than 0). Safe Withdrawal Model requires positive return rate.': 'Please enter a valid return rate (must be greater than 0%)',
      'Annual return rate must be at least 0.1% to avoid precision issues in calculations.': 'Return rate must be at least 0.1% to ensure accurate calculations',
      'Calculation overflow: Required assets exceeds safe integer limit. Try a smaller income or higher return rate.': 'The calculated amount is too large. Please try smaller values.',
      'Calculation overflow: Withdrawal amount exceeds safe integer limit.': 'The calculated amount is too large. Please try smaller values.',
      'Number of years must not exceed 100 to prevent performance issues and calculation overflow.': 'Please enter a projection period of 100 years or less',
    }
          ...
          <div className="space-y-4">
            ...
            <div className="p-6 bg-green-50 border border-green-200 rounded-xl">
              ...
              <p className="text-sm text-green-600 mt-2">
                You need <strong>{formatAmount(requiredAssets)}</strong> in retirement assets to safely withdraw 
                <strong>{formatAmount(parseCurrencyToCents(monthlyIncomeInput))}/month</strong> 
                at a <strong>{parsePercentageToDecimal(annualReturnRateInput) * 100}%</strong> annual return rate.
              </p>
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-medium ${retirementInsights.onTrack ? 'text-green-600' : 'text-orange-600'}`}>
                      {retirementInsights.onTrack ? '✓ On Track' : '⚠ Needs Attention'}
                    </span>
                  </div>
                  {retirementInsights.yearsToGoalAtCurrentRate !== null && retirementInsights.yearsToGoalAtCurrentRate > 0 && (
                    <div className="flex justify-between items-center">
