...
import { useProfilesWithActive, useProfileSwitcher, useHasMultipleProfiles } from '@/hooks/useActiveProfile'
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        aria-haspopup="true"
      >
        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-lg ${getProfileColor(activeProfile.id)}`}>
        ...
        <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-gray-200 py-2 z-50">
          ...
          <div className="max-h-80 overflow-y-auto">
            {profiles.map((profile) => (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-lg ${getProfileColor(profile.id)}`}>
                  {getProfileIcon(profile.id)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{profile.name}</p>
                  <p className="text-xs text-gray-500 truncate">{profile.description || 'No description'}</p>
                </div>
...
export function SwitchProfileSimple() {
  ...
  return (
    ...
    >
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-lg ${color}`}>
        {icon}
      </div>
