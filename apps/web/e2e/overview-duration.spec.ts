...
// Currency mode defaults to `none`, so amounts print as (cents/100).toFixed(2):
// 120000c monthly → 14400.00 annually, 1200.00 monthly.
...
test('overview defaults to Annually and re-expresses figures when the duration changes', async ({
  ...
  await expect(incomeValue).toHaveText('14400.00')
  ...
  await expect(incomeValue).toHaveText('1200.00')
})
